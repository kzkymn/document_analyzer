...
class TextExtractor:
    ...
    def extract_condition_driven_facts(
        self, conditions: List[PairCheckItem], text: str, source: Optional[str] = None
    ) -> List[Tuple[PairCheckItem, List[PairCheckItem]]]:
        """
        条件駆動型のファクト抽出を行う。
        各条件について、テキストから関連するファクトを抽出する。
        ...
        Returns:
            条件とそれに関連するファクトのリストのタプルのリスト
        """
        self.logger.info("条件駆動型のファクト抽出を開始します")

        result = []
        for condition in conditions:
            self.logger.info(f"条件「{condition.text}」に関連するファクトを抽出します")

            # 条件に関連するファクトを抽出するプロンプトを生成
            prompt = self._get_condition_driven_fact_extraction_prompt(
                condition.text, text
            )
            response = self.llm_processor.call_llm(prompt)
            facts_dict = self._parse_extraction_response(response)
            ...
            for item in fact_items:
                if item.parent_id is not None:
                    # 親アイテムを探す
                    parent = next(
                        (p for p in fact_items if p.id == item.parent_id), None
                    )
                    if parent:
                        if parent.children is None:
                            parent.children = []
            ...
            result.append((condition, fact_items))
            self.logger.info(
                f"条件「{condition.text}」に関連するファクトを{len(fact_items)}個抽出しました"
            )

        self.logger.info(
            f"{len(conditions)}個の条件について、関連するファクトを抽出しました"
        )
        return result
    ...
    def _get_condition_driven_fact_extraction_prompt(
        self, condition: str, text: str
    ) -> str:
        """
        条件駆動型のファクト抽出用のプロンプトを取得する。
        ...
        Returns:
            プロンプト
        """
        # 条件を分析して適切なファクト抽出の粒度を決定
        granularity_instructions = self._analyze_condition_for_granularity(condition)

        prompt = f"""
        あなたは文書分析の専門家です。以下の条件を検証するために、テキストから関連するファクト（事実）を抽出してください。
        ...
        # 指示
        1. テキストから、上記の条件を満たしているかを検証するために必要なファクト（事実）を抽出してください。
        2. 抽出するファクトは、条件との関連性が明確であるべきです。
        3. 条件の分析に基づく、ファクトの粒度に関する特別な指示：
        {granularity_instructions}
        4. ファクトの階層構造や関連性が明確な場合は、親子関係を表現してください。
        5. 1つのファクトが複数行にわたる場合は、それを1つのファクトとして扱ってください。
        6. 条件に関連するファクトが見つからない場合は、空のリストを返してください。
        ...
        - 各ファクトには一意のIDを割り当ててください。
        - 親ファクトがない場合は、parent_idをnullにしてください。
        - 子ファクトの場合は、parent_idに親ファクトのIDを指定してください。
        - textにはファクトの全文を含めてください。複数行にわたる場合も1つのテキストとして扱ってください。
        - 条件に関連するファクトが見つからない場合は、空の配列 [] を返してください。
        """
        return prompt
