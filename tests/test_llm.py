...
from document_analyzer.core.processor import AnalysisResult, ComplianceStatus, Evidence, Recommendation
...
class TestBaseLLMProcessor(unittest.TestCase):
    ...
    def test_generate_prompt_from_template(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as temp_template:
            template_content = "テスト参照: {reference_text}\nテスト対象: {file_content}"
...
class TestGeminiProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # APIキーのモック
        self.api_key_patcher = mock.patch("document_analyzer.utils.config.config.get_gemini_api_key")
